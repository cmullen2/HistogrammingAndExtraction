...
//   THSHisto::LoadWeights("WEIGHTFILE_DIR+NAME","WEIGHTNAME");
    THSHisto::LoadWeights("/w/work1/home/chris/LatestAnalysisRuns/Data/DataDec17ChargedPion/HaspectOutput/PiMinusP/Fitting/PiMinusPWeightsSum/Coplan/WeightsBothTotal.root","HSsWeights");
...
//if(*W>1040 &&  *Costh<0.8 ){
  ...
  if(*PolErrs==0){
	if(*AnyErrs==0){
			if(*W>950 && *W<1850){
				if(*PimTagDiffTime<20 && *PimTagDiffTime>-80){
  ...
  FindHist("Timing")->Fill(*PimTagDiffTime,fWeight);//1D
  ...
  ((TH2F*)FindHist("TimingVsW"))->Fill(*PimTagDiffTime,*W,fWeight);
  ...
  ((TH2F*)FindHist("CoplanarityVsTiming"))->Fill(*Coplanarity,*PimTagDiffTime,fWeight);
  ((TH2F*)FindHist("CoplanarityVsPhi"))->Fill(*Coplanarity,*Phi,fWeight);
  ((TH2F*)FindHist("TimingVsPhi"))->Fill(*PimTagDiffTime,*Phi,fWeight);
