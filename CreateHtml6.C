void CreateHtml6() {
  ...
  if (doBins) {
	...
	while ((ebinKey=(TKey*)nextEbinDir())) {
	  ...
	  while ((polbinKey=(TKey*)nextPolbinDir())){
	    ...
	    while ((histKey=(TKey*)nextHist())) {
	      ...
	      can->SaveAs("/home/chris/Docs/haspy/LatestFitting/Jan8th2017/Histogrammer/outhtml2/"+imFileName);
	      can->Clear();	
	     ...
	     // if (storeHistNames) histNames.push_back("PhiAsym");
	      //cout<< "Apple " << IsPolPos<< endl;
		...
		if(storeHistNames) histNames.push_back(histAsymName);
		//if(storeHistNames) cout << histAsymName << " Banana    " <<  endl;
    ...
    string Bottomlevel[2] = {"PolStateD-0.50_","PolStateD0.50_"};
		
    // all bins for each histogram
    for (int histNum=0; histNum < histNames.size(); histNum++) {
//cout<< histNames[histNum] << endl;
      for (int j=0; j<2; j++) {
	TString PolName = Bottomlevel[j];
	if (histNames[histNum]=="PhiAsym" && PolName=="PolStateD0.50_"){  // wrong
	break;
}
	  for (int i=0; i<3; i++) {
		...
		writeHead = false;
	      ...
	      //cout << histNames[histNum] << " BEFORe the IF " << endl;
	      while ((ebinKey=(TKey*)nextEbinDir())) {
		if(histNames[histNum]=="PhiAsym"){
		  cout << "MADE IT TO THE PHI ASYM " << endl;
		  TString imFileName = dirs[i]+(TString)tbinKey->GetName()+(TString)ebinKey->GetName()+histNames[histNum]+".gif";
		  outHtml << "<td><image src=\""+imFileName+"\" width=\"80%\"></image>" << endl;

		}
		else{
		  TString imFileName = dirs[i]+(TString)tbinKey->GetName()+(TString)ebinKey->GetName()+PolName+histNames[histNum]+".gif";
		  outHtml << "<td><image src=\""+imFileName+"\" width=\"80%\"></image>" << endl;
